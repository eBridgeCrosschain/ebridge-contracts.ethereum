...
// import {CrossChainLibrary} from "./libraries/CrossChainLibrary.sol";
...
contract BridgeInImplementation is ProxyStorage {
    ...
    using BridgeInLibrary for BridgeInLibrary.ChainMapping;
    using BridgeInLibrary for BridgeInLibrary.RequestData;
    ...
    function setBridgeOutAndLimiter(
        address _bridgeOut,
        address _limiter
    ) external onlyWallet {
        require(
    ...
    function addToken(Token[] calldata tokens) public onlyWallet {
        require(
            tokenList.length().add(tokens.length) <= MaxTokenCount &&
                tokens.length <= MaxTokenCountPerAddOrRemove,
            "token count exceed"
        );
    ...
    function setCrossChainConfig(
        BridgeInLibrary.CrossChainConfig calldata config
    ) external {
    ...
    function generateReceipt(
        ...
        string memory receiptId = BridgeInLibrary._generateReceiptId(
            tokenKey,
        ...
        ownerToReceiptsIndexMap[msg.sender][tokenKey] = ownerToReceiptsIndexMap[
            msg.sender
        ][tokenKey].add(1);
        ownerToReceiptIdMap[msg.sender][tokenKey][index] = receiptId;

        bytes memory data = BridgeInLibrary.encodeReceipt(
            token,
            msg.sender,
            amount,
            targetChainId,
        ...
        BridgeInLibrary.RequestData memory request = BridgeInLibrary
            .RequestData({
                oracleContract: oracleContract,
                receiver: targetAddress,
                targetChainId: chainMapping.stringToIntChainId[targetChainId],
                data: data,
                amount: amount
            });
        BridgeInLibrary.processAndSendRequest(request);
    ...
    function getSendReceiptIndex(
        ...
        require(tokens.length == targetChainIds.length, "invalid input");
    ...
    function getSendReceiptInfos(
        ...
        bytes32 tokenKey = _getTokenKey(token, targetChainId);
        uint256 lastIndex = tokenReceiptIndex[tokenKey];
        if (lastIndex == 0) {
            return _receipts;
        }
        require(
            endIndex <= lastIndex && fromIndex > 0 && fromIndex <= endIndex,
            "Invalid input"
        );
        ...
        for (uint256 i = 0; i < length; i++) {
            _receipts[i] = receiptIndexMap[tokenKey][fromIndex + i];
        }
        return _receipts;
    }
