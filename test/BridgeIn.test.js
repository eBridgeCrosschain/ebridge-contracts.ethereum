...
describe("BridgeIn", function () {
    async function deployBridgeInFixture() {
        ...
        const bridgeInProxy = await BridgeIn.deploy(multiSign.address, weth.address, pauseController.address, limiter.address, tokenPool.address, bridgeInImplementation.address);
        ...
        await bridgeIn.connect(multiSign).setBridgeOut(bridgeOutMock.address);
        let configs = [{
